...
# OpenMP
# On macOS with Apple Silicon (M1/M2/M3/M4, arm64), Apple's built-in clang does NOT
# support -fopenmp. You must use Homebrew LLVM + libomp instead.
# Install: brew install llvm libomp
# Then set: export CC=/opt/homebrew/opt/llvm/bin/clang
...
macx {
   # Apple Silicon / Homebrew LLVM: use -Xpreprocessor + libomp
   # Assumes Homebrew is at /opt/homebrew (standard Apple Silicon path)
...
!win32 {
   ...
   # On macOS, suppress spurious "argument unused during compilation" warnings
   # that can arise when -Xpreprocessor flags propagate to the linker step.
...
#    rm -f .qmake.cache .qmake.stash && BOOST_ROOT=/opt/homebrew/opt/boost /opt/homebrew/opt/qt@5/bin/qmake ../main.pro
...
#
# 2. Set environment (add to ~/.zshrc):
...
# 3. Build:
#      mkdir build && cd build
#      BOOST_ROOT=/opt/homebrew/opt/boost /opt/homebrew/opt/qt@5/bin/qmake ../main.pro
#      make -j$(sysctl -n hw.ncpu)
#
# 4. Run (required macOS flags):
